...
const normalizeSensorType = (sensorType: unknown): "real" | "simulation" => {
  return String(sensorType || "").toLowerCase() === "real"
    ? "real"
    : "simulation";
};
...
export const useSensors = () => {
  ...
  const fetchSensors = useCallback(async () => {
    ...
    try {
      ...
      const mappedSensors: Sensor[] = data.map((s: any) => {
        ...
        const status =
          sensorType === "real" && !hasReading ? "hors ligne" : statusFromApi;
        ...
        return {
          ...
          isLive:
            sensorType === "real" ? Boolean(s.is_live ?? hasReading) : true,
          sensorType,
          hasReading,
      ...
      setError(
        err.response?.data?.error || "Erreur lors du chargement des capteurs",
      );
    } finally {
      setIsLoading(false);
    }
      ...
      setSensors((prev) =>
        prev.map((sensor) =>
          sensor.id === normalizedSensorId
            ? (() => {
                const nextStatus = normalizeSensorStatus(
  ...
  const createSensor = async (
    ...
    try {
      const data = await apiClient.createSensor(
        name,
        location,
        sensorType,
        options,
      ...
      throw new Error(
        err.response?.data?.error || "Impossible de créer le capteur",
      );
    }
  };
  ...
  const updateSensor = async (
    ...
    try {
      ...
      throw new Error(
        err.response?.data?.error || "Impossible de mettre à jour le capteur",
      );
    }
  };
  ...
  const deleteSensor = async (sensorId: string) => {
    try {
      ...
      throw new Error(
        err.response?.data?.error || "Impossible de supprimer le capteur",
      );
    }
  };
  ...
  const addReading = async (
    ...
    try {
      ...
      throw new Error(
        err.response?.data?.error || "Impossible d'ajouter la mesure",
      );
    }
  };
