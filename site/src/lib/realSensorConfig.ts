...
const SENSOR_MODEL_LABELS: Record<string, string> = REAL_SENSOR_MODEL_OPTIONS.reduce(
  (acc, item) => {
...
const CONNECTION_METHOD_DESCRIPTIONS: Record<RealConnectionMethod, string> = {
  http_push: 'Connexion directe la plus simple: POST vers l\'endpoint d\'ingestion.',
  mqtt_bridge: 'Publiez sur votre broker MQTT; une passerelle relaie ensuite vers Aerium.',
  serial_gateway: 'L\'agent local lit les trames serie puis envoie les mesures en HTTP.',
  websocket_gateway: 'La passerelle recoit en WebSocket puis transfere vers l\'endpoint HTTP Aerium.',
};
...
export function getConnectionMethodDescription(method?: string | null): string {
  ...
  const key = method.toLowerCase() as RealConnectionMethod;
  return CONNECTION_METHOD_DESCRIPTIONS[key] || CONNECTION_METHOD_DESCRIPTIONS.http_push;
}
